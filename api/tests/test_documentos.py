...
def mock_storage_service():
    ...
    service.upload_private_file = AsyncMock(side_effect=lambda *args, **kwargs: f"cloudinary_id_{uuid.uuid4()}")
...
@pytest.mark.anyio
async def test_criar_documento_tamanho_excedido(db_client, mock_pdf_service, mock_storage_service):
...
@pytest.mark.anyio
async def test_criar_documento_tipo_invalido(db_client, mock_pdf_service, mock_storage_service):
    service = DocumentoService(db_client, mock_pdf_service, mock_storage_service)
...
@pytest.mark.anyio
async def test_gerar_url_download_com_flag_cloudinary(db_client, mock_pdf_service, mock_storage_service, pdf_minimal):
    service = DocumentoService(db_client, mock_pdf_service, mock_storage_service)
    
    # Mockando a URL de retorno para ser uma do Cloudinary
    mock_storage_service.generate_signed_url.return_value = "https://res.cloudinary.com/demo/image/upload/v1/private_id"

    # Criar um documento para testar o download
    novo_doc = await service.criar_documento(
        DocumentoCreate(titulo="Doc Teste", categoria="Geral"),
        pdf_minimal, "teste.pdf", 1, 1
    )
...
@pytest.mark.anyio
async def test_deletar_documento(db_client, mock_pdf_service, mock_storage_service, pdf_minimal):
    service = DocumentoService(db_client, mock_pdf_service, mock_storage_service)
    
    # Criar documento
    novo_doc = await service.criar_documento(
        DocumentoCreate(titulo="Doc para Delete", categoria="Geral"),
        pdf_minimal, "delete.pdf", 1, 1
    )
