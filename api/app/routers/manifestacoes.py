...
async def criar_manifestacao(dados: ManifestacaoCreate, usuario_logado=Depends(get_current_user), db: Prisma = Depends(get_prisma)):
    ...
    return await ManifestacaoController.criar_manifestacao(dados=dados, autor=nome, db=db)
...
@router.put("/atualizar-manifestacao/{manifestacao_id}", response_model=ManifestacaoResponse)
async def atualizar_manifestacao(manifestacao_id: int, dados: ManifestacaoUpdate, db: Prisma = Depends(get_prisma)):
    autor = "sindico"
    return await ManifestacaoController.atualizar_manifestacao(manifestacao_id=manifestacao_id, dados=dados, autor=autor, db=db)

@router.delete("/deletar-manifestacao/{manifestacao_id}", status_code=status.HTTP_200_OK)
async def deletar_manifestacao(manifestacao_id: int, db: Prisma = Depends(get_prisma)):
    await ManifestacaoController.deletar_manifestacao(manifestacao_id=manifestacao_id, db=db)
