...
async def listar_historico(
    ...
    tasks.append(db.visitante.find_many(where=where_visitante, order={"criado_em": "desc"}, take=take_per_query))
    
    # 2. Bilhetes
    where_bilhete = {}
    if not is_gestao and usuario.morador:
        where_bilhete = {"morador_id": usuario.morador.id}
    tasks.append(db.bilhetes.find_many(where=where_bilhete, order={"criado_em": "desc"}, take=take_per_query))
    
    # 3. Reservas
    where_reserva = {}
    if not is_gestao:
        where_reserva = {"usuario_id": usuario_id}
    tasks.append(db.reserva.find_many(where=where_reserva, include={"espaco": True}, order={"data_reserva": "desc"}, take=take_per_query))
    
    # 4. Manifestações
    where_manifestacao = {}
    if not is_gestao and usuario.morador:
        where_manifestacao = {"morador_id": usuario.morador.id}
    tasks.append(db.manifestacao.find_many(where=where_manifestacao, order={"criado_em": "desc"}, take=take_per_query))
    ...
    for v in visitantes:
        historico_unificado.append(HistoricoItem(
            id=f"vis_{v.id}",
