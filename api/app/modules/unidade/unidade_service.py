...
class UnidadeService:
    ...
    async def registrar_unidade(dados: UnidadeCreate, db: Prisma):
        ...
        # 2. Criar unidade
        unidade = await db.unidade.create(data=dados.model_dump())

        unidade_criada = await db.unidade.find_unique(
            where={"id": unidade.id}, include={"moradores": True}
        )
    ...
    async def atualizar_unidade(unid_id: int, dados: UnidadeUpdate, db: Prisma):
        ...
        unidade_existente = await db.unidade.find_unique(where={"id": unid_id})
        ...
        if dados.unidade:
            unid_igual = await db.unidade.find_first(where={"unidade": dados.unidade})
        ...
        unidade = await db.unidade.update(
            where={"id": unid_id}, data=dados.model_dump(exclude_unset=True)
        )
        return unidade
    ...
    async def listar_unidades(condominio_id: int, db: Prisma):
        unidades = await db.unidade.find_many(
            where={"condominio_id": condominio_id}, include={"moradores": True}
        )
        return unidades

    @staticmethod
    async def buscar_unidade_por_id(unid_id: int, db: Prisma):
        return await db.unidade.find_unique(where={"id": unid_id})

    @staticmethod
    async def cadastro_massivo(
        dados: UnidMassCreation, db: Prisma, quem_criou: int | None = None
    ):
        ...
        ja_existentes = await db.unidade.find_many(where={"unidade": {"in": nomes}})
        nomes_existentes = {u.unidade for u in ja_existentes}

        a_criar = [c for c in candidatas if c["unidade"] not in nomes_existentes]
        ignoradas = [
            c["unidade"] for c in candidatas if c["unidade"] in nomes_existentes
        ]
        ...
        if a_criar:
            ...
            criadas = await db.unidade.find_many(
                where={
                    "unidade": {"in": [p["unidade"] for p in payload]},
                    "condominio_id": dados.condominio_id,
                },
