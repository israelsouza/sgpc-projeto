...
class CondominioService:
    ...
    async def buscar_condominio_por_id(cond_id: int, db: Prisma):
        return await db.condominio.find_unique(where={"id": cond_id})

    @staticmethod
    async def atualizar_condominio(cond_id: int, dados: CondominioUpdate, db: Prisma):
        existente = await db.condominio.find_unique(where={"id": cond_id})
        if not existente:
            return None

        if dados.cnpj:
            outro = await db.condominio.find_first(where={"cnpj": dados.cnpj})
        ...
        return await db.condominio.update(
            where={"id": cond_id}, data=dados.model_dump(exclude_unset=True)
        )
